...
exports.getProperty = function(key, defaultValue) {
    ...
    let promise = promiseCache[key] = co(function* () {
        if (!zk.isInit) {
            yield zk.init()
        }
        ...
        // 存在key，从zk读取数值加入缓存
        let value = normalValue = yield zk.client.getAndWatch(path(key), res => {
            if (res.data !== null && !swim) {
                cache[key] = res.data
            }
            normalValue = res.data
        })
...
exports.removeListener = function(key, listener) {
    if (!listenerCache[key]) {
        return
    }

    let idx = listenerCache[key].indexOf(listener)
    if (idx > -1) {
        listenerCache[key].splice(idx, 1)
    }
}
